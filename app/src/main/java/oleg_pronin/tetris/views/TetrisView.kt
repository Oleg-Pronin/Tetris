package oleg_pronin.tetris.views
...
import oleg_pronin.tetris.GameActivity
...
import oleg_pronin.tetris.models.Block
...
class TetrisView : View {
    ...
    fun setGameCommand(move: AppModel.Motions) {
        if (model != null && (model?.currentState == AppModel.Statuses.ACTIVE.name)) {
            if (AppModel.Motions.DOWN == move) {
                ...
                invalidate() // в конечном итоге приводит в вызому метода onDraw
                return
            }
    ...
    private fun updateScore() {
        activity?.updateCurrentScore("${model?.score}")
        activity?.updateHighScore("${activity?.appPreferences?.getHighScore()}")
    }
